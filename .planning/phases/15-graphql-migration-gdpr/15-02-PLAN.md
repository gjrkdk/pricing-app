...
must_haves:
  truths:
    ...
    - "Draft Order creation uses GraphQL draftOrderCreate mutation (not REST)"
    - "Product fetching uses GraphQL (App Bridge ResourcePicker, not REST Admin API)"
...
The GDPR request logging at the top of the action (the `prisma.gdprRequest.create()` call) should remain — this logs the incoming webhook payload for audit trail BEFORE enqueueing the async job.
...
<task type="auto">
  <name>Task 3: Verify GraphQL usage for Draft Orders and product fetching</name>
  <files>app/services/draft-order.server.ts</files>
  <action>
This is a code inspection and verification task, NOT a code change task. The codebase already uses GraphQL for Draft Orders and product fetching. This task proves it for phase success criteria 1 and 2.

**Step 1: Verify Draft Order uses GraphQL draftOrderCreate mutation**

Read `app/services/draft-order.server.ts` and confirm:
- It uses `admin.graphql()` to call the Shopify GraphQL Admin API
- The GraphQL mutation string contains `draftOrderCreate`
- There are NO REST API calls (no `admin.rest`, no `/admin/api/*/draft_orders.json` URLs)

**Step 2: Verify product fetching uses GraphQL**

Product fetching in this app is done via Shopify App Bridge's ResourcePicker component (used in the admin UI for product assignment). App Bridge ResourcePicker uses GraphQL internally — there is no REST product fetching endpoint in the app code.

Confirm by searching the codebase:
- `grep -r "admin.rest" app/` should return NO results for product fetching or draft orders
- `grep -r "rest.Product" app/` should return NO results
- `grep -r "/admin/api/" app/` should return NO REST API URL patterns for products or draft orders

**Step 3: Document findings**

Add a brief comment at the top of `app/services/draft-order.server.ts` (below existing imports) confirming GraphQL usage:
```typescript
...
If any REST Admin API calls are found for Draft Orders or product fetching, report them as a blocker. (Research indicates none exist.)
  </action>
  <verify>
1. Read app/services/draft-order.server.ts: confirm `admin.graphql()` and `draftOrderCreate` mutation present
2. Run: `grep -r "admin\.rest" app/ || echo "No REST API calls found"` — expect no results
3. Run: `grep -r "/admin/api/.*draft_orders" app/ || echo "No REST draft order URLs found"` — expect no results
4. Run: `grep -r "/admin/api/.*products" app/ || echo "No REST product URLs found"` — expect no results
5. Confirm GraphQL comment added to draft-order.server.ts
  </verify>
  <done>Verified: Draft Order creation uses GraphQL draftOrderCreate mutation. Product fetching uses App Bridge ResourcePicker (GraphQL internally). No REST Admin API calls found for these operations. Phase success criteria 1 and 2 confirmed.</done>
</task>
...
7. draft-order.server.ts uses GraphQL draftOrderCreate mutation (not REST)
8. No REST Admin API calls exist for product fetching or draft order creation
</verification>
