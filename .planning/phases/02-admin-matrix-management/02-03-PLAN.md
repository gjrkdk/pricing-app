...
must_haves:
  ...
  artifacts:
    ...
    - path: "app/components/MatrixGrid.tsx"
      provides: "Custom HTML table-based spreadsheet grid for price matrix"
      min_lines: 80
  ...
  key_links:
    ...
    - from: "app/components/MatrixGrid.tsx"
      to: "HTML table with input elements"
      via: "custom spreadsheet grid with breakpoint headers on both axes"
      pattern: "<table"
---

<objective>
Build the matrix editor page with a spreadsheet-style editable grid using a custom HTML table. Merchants can edit prices inline, add/remove breakpoints, and save changes with full validation.
...
<task type="auto">
  <name>Task 1: Build MatrixGrid and UnsavedChangesPrompt components</name>
  <files>app/components/MatrixGrid.tsx, app/components/UnsavedChangesPrompt.tsx</files>
  <action>
**1. Create `app/components/MatrixGrid.tsx`:**

This component renders a custom HTML table-based spreadsheet grid for the price matrix. It receives breakpoints and cell data as props and calls back on changes.
...
**2. Create `app/components/UnsavedChangesPrompt.tsx`:**
